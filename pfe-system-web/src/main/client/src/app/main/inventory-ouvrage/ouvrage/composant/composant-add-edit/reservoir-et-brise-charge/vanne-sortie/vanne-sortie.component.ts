...
@Component({
  selector: 'app-vanne-sortie',
  templateUrl: './vanne-sortie.component.html',
  styleUrls: ['./vanne-sortie.component.scss'],
    animations: fuseAnimations
})
export class VanneSortieComponent implements OnInit{
    ...
    createForm(composant): FormGroup {
        let obj = {
            ...
            equipementType:['VanneDistribution'],
    ...
    initForm(action){
        if (action == 'edit') {
            ...
            if(this.typeEquipement(this.equipements)) {
                this.equipement = new EquipementHydroMeca(this.typeEquipement(this.equipements));
                this.vanneSortieForm = this.createForm(this.equipement);
            }
    ...
    typeEquipement(composants):EquipementHydroMeca{
        for (var i in composants){
            if (composants[i].equipementType =='VanneDistribution') return composants[i];
        }
        return null;
    }
