...
export class InventoryAddComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        this.route.data.pipe(takeUntil(this._unsubscribeAll)).subscribe(
            (response) => {
                this.date = new Date();
                this.unitId = response.data[4].id;
    ...
    calCode() {
        ...
        this.code = this.unitId + month + this.date.getFullYear().toString().substr(2, 2)
    }

    createInventoryForm(): FormGroup {
        let obj = {
            code: [{ value: this.inventory.code, disabled: true }, Validators.required],
