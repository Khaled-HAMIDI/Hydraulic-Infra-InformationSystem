...
import {GroupeElectroPompe} from "../../../../../model/composant.model";
...
export class GroupeElecPompPompShowComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.composantShowService.loadPompe(this.route.snapshot.params['code']).then(
            (composant) => {
    ...
    initComposant(composant) {
        this.groupe= new GroupeElectroPompe(composant);
