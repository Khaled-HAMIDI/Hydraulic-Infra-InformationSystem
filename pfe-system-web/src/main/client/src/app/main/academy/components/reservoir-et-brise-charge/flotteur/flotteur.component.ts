import { Component, OnInit, EventEmitter, Output } from '@angular/core';
import { FormGroup, FormBuilder, Validators } from '@angular/forms';
import { fuseAnimations } from '@fuse/animations';
import { ActivatedRoute, Router } from '@angular/router';
import { FuseTranslationLoaderService } from '@fuse/services/translation-loader.service';
import { locale as french } from '../i18n/fr';
import { locale as arabic } from '../i18n/ar';
import { Subject } from "rxjs";
import { takeUntil } from "rxjs/operators";
import { EquipementHydroMeca } from 'app/main/model/composant.model';
import { ComposantSaveService } from 'app/main/inventory-ouvrage/ouvrage/composant/composant-save.service';

@Component({
    selector: 'testapp-flotteur',
    templateUrl: './flotteur.component.html',
    styleUrls: ['./flotteur.component.scss'],
    animations: fuseAnimations
})
export class TestFlotteurComponent implements OnInit {

    private _unsubscribeAll: Subject<any>;
    exist: boolean;
    add: boolean;
    equipement: EquipementHydroMeca;
    equipements: EquipementHydroMeca[];
    equipementHydroMecaForm: FormGroup;

    @Output() validateEvent = new EventEmitter<string>();

    constructor(
        private composantService: ComposantSaveService,
        private formBuilder: FormBuilder,
        private router: Router,
        private route: ActivatedRoute,
        private fuseTranslationLoader: FuseTranslationLoaderService
    ) {
        // Set the default
        this.fuseTranslationLoader.loadTranslations(french, arabic);
        this._unsubscribeAll = new Subject();
        this.add = true;
        this.exist = true;
        this.equipement = new EquipementHydroMeca();
    }

    /**
     * On init
     */
    ngOnInit(): void {
        this.route.data.pipe(takeUntil(this._unsubscribeAll)).subscribe(
            (response) => {
                if (response.action == 'edit') this.equipements = response.data[15];
                this.initForm(response.action);
            },
            (error) => {
                console.log(error);
            }
        );

    }


    createForm(composant): FormGroup {
        let obj = {
            typeComposant: ['ComposantHydroMecanique'],
            equipementType: ['Flotteur'],
            dn: [composant.dn, Validators.required],
            pn: [composant.pn, Validators.required],
            state: [composant.state, Validators.required],
            materiaux: [composant.materiaux, Validators.required],
            lieuImplantation: [composant.lieuImplantation, Validators.required],
            type: [composant.type, Validators.required],
            number: [composant.number, Validators.required]
        };

        return this.formBuilder.group(obj);

    }

    initForm(action) {
        if (action == 'edit') {
            this.add = false;

            if (this.typeEquipement(this.equipements)) {
                this.equipement = new EquipementHydroMeca(this.typeEquipement(this.equipements));
                this.equipementHydroMecaForm = this.createForm(this.equipement);
            }
            else this.exist = false;

        }
        else {
            this.equipement = new EquipementHydroMeca();
            this.equipementHydroMecaForm = this.createForm(this.equipement);
        }
    }

    typeEquipement(composants): EquipementHydroMeca {
        for (var i in composants) {
            if (composants[i].equipementType == 'Flotteur') return composants[i];
        }
        return null;
    }

    onSave(): void {
        this.validateEvent.emit("to-next-step");
        // const equipementHydroMeca = this.equipementHydroMecaForm.getRawValue();

        // this.composantService.saveHydroMeca(equipementHydroMeca, this.route.snapshot.params['code'])
        //     .then((response) => {
        //         console.log("It worked");
        //         this.validateEvent.emit("to-next-step");
        //     },
        //         (error) => {
        //             console.log("No")
        //         });
    }

}
